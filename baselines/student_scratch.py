...
class MobileNetV3(torch.nn.Module):
    def __init__(self, num_classes):
        ...
        self.mobilenet_v3.classifier[3] = torch.nn.Linear(1024, num_classes)

    def forward(self, x):
        return self.mobilenet_v3(x)
    
model = MobileNetV3(num_classes=n_classes)
...
def test_multiple_outputs(model, test_loader, device, validation = False):
    ...
    if validation:
        with open("validation_accuracy_f1_scratch.txt", "w") as f:
            print("f1:", f1, file=f)
            print(f"Validation Accuracy: {accuracy:.2f}%", file=f)
    else:
        with open("test_accuracy_f1_scratch.txt", "w") as f:
            print("f1:", f1, file=f)
...
def train_model(model, criterion, optimizer, train_loader, val_loader, epochs, device):
    ...
    torch.save(best_model, 'scratch_model.pth')
...
model = torch.load('scratch_model.pth')
